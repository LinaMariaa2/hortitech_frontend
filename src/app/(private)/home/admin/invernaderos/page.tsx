...
export default function InvernaderosPage() {
    ...
    const obtenerInvernaderos = async (responsableId: number | null = filtroResponsableId) => {
        setCargando(true);
        try {
            let url = `${process.env.NEXT_PUBLIC_API_URL}/api/invernadero`;
            if (responsableId) {
                // Usa la nueva ruta para filtrar por operario
                url = `${process.env.NEXT_PUBLIC_API_URL}/api/invernadero/operario/${responsableId}`;
            }
    ...
    // Ejecuta obtenerInvernaderos cuando el filtroResponsableId cambie
    useEffect(() => {
    ...
    const handleFormSubmit = async () => {
        ...
        try {
            ...
            if (editarModo) {
                await axios.put(`${process.env.NEXT_PUBLIC_API_URL}/api/invernadero/${editarModo}`, payload);
            } else {
                await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/api/invernadero`, {...payload, estado: 'activo'});
            }
    ...
    const cambiarEstado = (id: number, nuevoEstado: string) => {
        const onConfirm = async () => {
            try {
                ...
                await axios.patch(`${process.env.NEXT_PUBLIC_API_URL}/api/invernadero/${ruta}/${id}`);
                await obtenerInvernaderos();
    ...
    const eliminarInvernadero = (id: number) => {
        setModalConfirm({
            ...
            onConfirm: async () => {
                try {
                    await axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/api/invernadero/${id}`);
                    await obtenerInvernaderos();
    ...
    useEffect(() => {
        ...
        const debounce = setTimeout(async () => {
            try {
                const response = await axios.get(
                    `${process.env.NEXT_PUBLIC_API_URL}/api/persona?filtro=${encodeURIComponent(busquedaFiltro)}`,
                    { signal: controller.signal }
                );
                setResponsablesFiltro(Array.isArray(response.data) ? response.data : []);
            } catch (error) {
                if (!axios.isCancel(error)) {
                    console.error("Error al obtener responsables para filtro:", error);
                }
            }
        }, 300);
    ...
    const handleSelectResponsableFiltro = (r: Responsable) => {
        ...
        setResponsablesFiltro([]); // Cierra el dropdown
    };
        ...
        <main className="w-full bg-slate-50 min-h-screen p-6 sm:p-8">
            ...
            <div className="mb-8 p-4 bg-white rounded-xl shadow-sm border border-slate-200">
                <div className="relative">
                    ...
                    <input 
                        type="text" 
                        placeholder="Buscar invernaderos por responsable (nombre o correo)..." 
                        ...
                        className="w-full border-slate-300 p-3 pl-10 pr-10 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500" 
                    />
                    {filtroResponsableId && (
                        <button onClick={handleClearFiltro} className="absolute right-3.5 top-1/2 -translate-y-1/2 p-1 text-slate-500 hover:text-red-500 rounded-full">
                            <X className="w-5 h-5" />
                        </button>
                    )}
                </div>
                {busquedaFiltro && responsablesFiltro.length > 0 && !filtroResponsableId && (
                    <ul className="absolute z-20 w-full md:w-1/3 border border-slate-200 mt-1 rounded-lg shadow-lg max-h-60 overflow-y-auto bg-white">
                        {responsablesFiltro.map((r) => (
                            <li key={r.id_persona} className="px-4 py-2 hover:bg-teal-50 cursor-pointer flex justify-between items-center" onClick={() => handleSelectResponsableFiltro(r)}>
                ...
                <p className="mt-3 text-sm text-slate-600">
                    {filtroResponsableId ? 
                        <span className="font-semibold text-teal-600">Filtro Activo: Mostrando invernaderos del responsable con ID {filtroResponsableId}.</span>
                        : 'Mostrando todos los invernaderos.'
                    }
                    {filtroResponsableId && (
                        <button onClick={handleClearFiltro} className="ml-2 text-red-500 hover:text-red-700 font-medium">
                            (Limpiar Filtro)
                        </button>
                    )}
                </p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {invernaderos.length === 0 ? (
                        <div className="col-span-full text-center py-10 bg-white rounded-xl border border-slate-200">
                            ...
                            <p className="text-slate-500">No se encontraron invernaderos {filtroResponsableId ? `para el responsable seleccionado.` : `en el sistema.`}</p>
                        </div>
                    ) : (
            ...
            {/* Modal de Creación/Edición (se mantiene sin cambios mayores) */}
            {modalOpen && (
